...
class ParallelMultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        k: torch.Tensor = self.key(x)  # (B, T, head_size * num_heads) I think
        # k = k.view(B, self.num_heads, T, -1)  # (B, num_heads, T, head_size) I think
        k = k.view(B, T, self.num_heads, self.head_size).transpose(1, 2)  # (B, num_heads, T, head_size)
        q: torch.Tensor = self.query(x)  # (B, T, head_size * num_heads) I think
        # q = q.view(B, self.num_heads, T, -1)  # (B, num_heads, T, head_size) I think
        q = q.view(B, T, self.num_heads, self.head_size).transpose(1, 2)  # (B, num_heads, T, head_size)
        # compute attention scores ("affinities")
        wei = (
            q @ k.transpose(-2, -1) * k.size(-1) ** -0.5
        )  # (B, num_heads, T, C) @ (B, num_heads, C, T) ---> (B, num_heads, T, T)
        ...
        # perform the weighted aggregation of the values
        v: torch.Tensor = self.value(x)  # (B, T, head_size * num_heads)
        # v = v.view(B, self.num_heads, T, -1)  # (B, num_heads, T, head_size)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # slight variation from original paper: layer norm applied before the transformation, called "prenorm formulation"
...
class BigramLanguageModel(nn.Module):
    def __init__(self):
        ...
        # Below line actually makes it worse on the short run
        # self.token_embedding_table.weight = self.lm_head.weight # https://paperswithcode.com/method/weight-tying

        # Below line gets us to the optimal results quicker
